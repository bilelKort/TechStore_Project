...
{
    QSqlQueryModel *model= new QSqlQueryModel();
    model->setQuery("SELECT * FROM SPONSOR WHERE CIN LIKE'%"+rech+"%'");
    return model;
...
QSqlQueryModel* sponsor::statistiques()
     {
         QSqlQueryModel* model= new QSqlQueryModel();
         model->setQuery("SELECT MAIL_S, COUNT(ID_S) FROM SPONSOR  GROUP BY MAIL_S ");
         model->setHeaderData(0,Qt::Horizontal,QObject::tr("Mail"));
         model->setHeaderData(1,Qt::Horizontal,QObject::tr("Nbr_Email"));
         model->setHeaderData(2,Qt::Horizontal,QObject::tr("Nbr_Etoil"));
         return model;
     }
